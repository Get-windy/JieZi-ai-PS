// 🌍 国际化包装层 - 包装上游 usage-render-overview.ts
...
function renderFilterChips(
  ...
  const daysLabel = selectedDays.length === 1 ? selectedDays[0] : t("usage.nDays", { count: selectedDays.length });
  const hoursLabel =
    selectedHours.length === 1 ? `${selectedHours[0]}:00` : t("usage.nHours", { count: selectedHours.length });
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">${t("usage.daily")} ${isTokenMode ? t("usage.tokens") : t("usage.cost")} ${t("usage.dailyUsage")}</div>
      </div>
