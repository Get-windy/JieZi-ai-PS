...
import { deleteSessionsAndRefresh, loadSessions } from "./controllers/sessions.ts";
...
export function renderTab(state: AppViewState, tab: Tab, opts?: { collapsed?: boolean }) {
  ...
  const collapsed =
    opts?.collapsed ??
    (state as unknown as { settings?: { navCollapsed?: boolean } }).settings?.navCollapsed;
  return html`
    <a
      href=${href}
    ...
    <div class="chat-controls">
      ...
      <button
        ...
        @click=${async () => {
          ...
          await (app as unknown as { updateComplete: Promise<void> }).updateComplete;
          app.resetToolStream();
          try {
      ...
      <button
        ...
        title=${
          disableThinkingToggle
            ? t("chat.controls.thinking.disabled")
            : t("chat.controls.thinking.toggle")
        }
      >
        ${icons.brain}
      </button>
...
export function resolveSessionOptionGroups(
  ...
  const hideCron = (state as unknown as { sessionsHideCron?: boolean }).sessionsHideCron ?? true;
...
function resolveAgentGroupLabel(state: AppViewState, agentIdRaw: string): string {
  ...
  const agent = (
    (
      state as unknown as {
        agentsList?: {
          agents?: Array<{ id: string; identity?: { name?: string }; name?: string }>;
        };
      }
    ).agentsList?.agents ?? []
  ).find((entry) => normalizeLowercaseStringOrEmpty(entry.id) === normalized);
  const name =
...
function renderChatModelSelect(state: AppViewState) {
  ...
  const disabled =
    !state.connected ||
    busy ||
    ((state as unknown as { chatModelsLoading?: boolean }).chatModelsLoading &&
      options.length === 0) ||
    !state.client;
  const selectedLabel =
    currentOverride === ""
  ...
  const defaultLevel =
    provider && model
      ? resolveThinkingDefaultForModel({
          provider,
          model,
          catalog: (state as unknown as { chatModelCatalog?: unknown[] }).chatModelCatalog ?? [],
        })
      : "off";
  return {
    currentOverride,
...
async function switchChatModel(state: AppViewState, nextModel: string) {
  ...
  const prevOverride = (state as unknown as { chatModelOverrides: Record<string, unknown> })
    .chatModelOverrides[targetSessionKey];
  state.lastError = null;
  (state as unknown as { chatModelOverrides: Record<string, unknown> }).chatModelOverrides = {
    ...(state as unknown as { chatModelOverrides: Record<string, unknown> }).chatModelOverrides,
        ...
        <div class="chat-controls">
          ...
          <div class="chat-controls__thinking">
            <button
              ...
              aria-pressed=${showThinking}
              title=${t("chat.controls.thinking.toggle")}
            >
              ${icons.brain}
            </button>
            <button
              ...
              title=${t("chat.controls.toolCalls") ?? "Tool Calls"}
            >
              ${toolCallsIcon}
            </button>
            <button
              ...
              aria-pressed=${focusActive}
              title=${t("chat.controls.focus.toggle")}
            >
              ${focusIcon}
            </button>
          </div>
...
export function renderTopbarThemeModeToggle(state: AppViewState) {
  const themeMode = (state as unknown as { themeMode?: ThemeMode }).themeMode;
  const setThemeMode = (
    state as unknown as { setThemeMode?: (mode: ThemeMode, ctx?: unknown) => void }
  ).setThemeMode;
  ...
  const applyMode = (mode: ThemeMode, e: Event) => {
    ...
    setThemeMode?.(mode, { element: e.currentTarget as HTMLElement });
  };
        ...
        (opt) => html`
          <button
            ...
            class="topbar-theme-mode__btn ${
              opt.id === themeMode ? "topbar-theme-mode__btn--active" : ""
            }"
            title=${opt.label}
            aria-label="Color mode: ${opt.label}"
            aria-pressed=${opt.id === themeMode}
...
export function renderSidebarConnectionStatus(state: AppViewState) {
  const label = state.connected ? t("common.connected") : t("common.offline");
