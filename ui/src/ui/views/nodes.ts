...
import { resolveConfigAgents, resolveNodeTargets } from "./nodes-shared.ts";
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t("nodes.list.button.loading") : t("nodes.list.button.refresh")}
        </button>
      </div>
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodes.list.no_nodes")}</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("nodes.devices.button.loading") : t("nodes.devices.button.refresh")}
        </button>
      </div>
      ${
        props.devicesError
...
function renderPendingDevice(req: PendingDevice, props: NodesProps) {
  ...
  const role = req.role?.trim()
    ? `${t("nodes.devices.role")}: ${req.role}`
    : `${t("nodes.devices.role")}: -`;
      ...
      <div class="list-meta">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button class="btn btn--sm primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            ${t("nodes.devices.button.approve")}
          </button>
          <button class="btn btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            ${t("nodes.devices.button.reject")}
          </button>
        </div>
      </div>
    </div>
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  const status = token.revokedAtMs
    ? t("nodes.devices.status.revoked")
    : t("nodes.devices.status.active");
        ...
        state.formMode === "raw"
          ? html`
              <div class="callout warn" style="margin-top: 12px">
                ${t("nodes.binding.form_mode_hint")}
              </div>
            `
          : nothing
      }
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("nodes.binding.load_config_hint")}</div>
          ...
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                <div class="list-main">
                  <div class="list-title">${t("nodes.binding.default_binding")}</div>
                  <div class="list-sub">${t("nodes.binding.default_binding_hint")}</div>
                </div>
                <div class="list-meta">
                  <label class="field">
                    ...
                    >
                      <option value="" ?selected=${defaultValue === ""}>${t("nodes.binding.any_node")}</option>
                      ${state.nodes.map(
                        (node) =>
                          html`<option
                            value=${node.id}
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.binding.no_nodes_available")}</div>
                        `
                      : nothing
                  }
                </div>
              ...
              ${
                state.agents.length === 0
                  ? html`
                      <div class="muted">${t("nodes.binding.no_agents_found")}</div>
                    `
