    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; margin-bottom: 12px;">
        <div>
          ...
          <div class="card-sub">
            ${props.result
              ? t("sessions.storePath", { path: props.result.path })
              : t("sessions.subtitle")}
          </div>
        </div>
      ...
      <div class="filters" style="margin-bottom: 12px;">
        <label class="field-inline">
          <span>${t("sessions.activeLabel")}</span>
          <input
            style="width: 72px;"
            placeholder="min"
        ...
        <label class="field-inline">
          <span>${t("sessions.limitLabel")}</span>
          <input
            style="width: 64px;"
            .value=${props.limit}
            @input=${(e: Event) =>
        ...
        <label class="field-inline checkbox">
          ...
          />
          <span>${t("sessions.globalLabel")}</span>
        </label>
        <label class="field-inline checkbox">
          <input
            type="checkbox"
          ...
          />
          <span>${t("sessions.unknownLabel")}</span>
        </label>
      </div>
        ...
        ${props.selectedKeys.size > 0
          ? html`
              <div class="data-table-bulk-bar">
                <span>${t("sessions.selected", { count: String(props.selectedKeys.size) })}</span>
                ...
                <th class="data-table-checkbox-col">
                  ${paginated.length > 0
                    ? html`<input
                        ...
                        aria-label=${t("sessions.selectAllPage")}
                      />`
                    : nothing}
                </th>
                ${sortHeader("key", t("sessions.keyCol"), "data-table-key-col")}
                <th>${t("sessions.labelCol")}</th>
                ${sortHeader("kind", t("sessions.kindCol"))} ${sortHeader("updated", t("sessions.updatedCol"))}
                ${sortHeader("tokens", t("sessions.tokensCol"))}
                <th>${t("sessions.compactionCol")}</th>
                <th>${t("sessions.thinkingCol")}</th>
                <th>${t("sessions.fastCol")}</th>
                <th>${t("sessions.verboseCol")}</th>
                <th>${t("sessions.reasoningCol")}</th>
              </tr>
            </thead>
            <tbody>
              ${paginated.length === 0
          ...
          ? html`
              <div class="data-table-pagination">
                <div class="data-table-pagination__info">
                  ...
                  ${t("sessions.of")} ${totalRows} ${t("sessions.rowsSuffix")}
                </div>
                <div class="data-table-pagination__controls">
                  ...
                  >
                    ${PAGE_SIZES.map((s) => html`<option value=${s}>${t("sessions.perPage", { count: String(s) })}</option>`)}
                  </select>
    ...
    html`<tr>
      ...
      <td>
        <div style="display: grid; gap: 6px;">
          ...
          >
            ${isExpanded ? t("sessions.hideCheckpoints") : t("sessions.showCheckpoints")}
          </button>
        </div>
      </td>
      <td>
            ...
            <td colspan="11" style="padding: 0;">
              ...
              >
                ${props.checkpointLoadingKey === row.key
                  ? html`<div class="muted">${t("sessions.loadingCheckpoints")}</div>`
                  : checkpointError
                    ? html`<div class="callout danger">${checkpointError}</div>`
                    : checkpointItems.length === 0
                      ? html`<div class="muted">
                          ${t("sessions.noCheckpoints")}
                        </div>`
                      : html`
                              ...
                              (checkpoint) => html`
                                ...
                                >
                                  ...
                                  ${checkpoint.summary
                                    ...
                                    : html`<div class="muted">${t("sessions.noSummary")}</div>`}
                                  <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                                    ...
                                    >
                                      ${t("sessions.branchFromCheckpoint")}
                                    </button>
                                    <button
                                      class="btn btn--sm"
                                      ?disabled=${props.checkpointBusyKey ===
