...
const THINK_LEVELS = [
  { value: "", label: () => t("sessions.level.inherit") },
  { value: "off", label: () => t("sessions.level.off") },
  { value: "minimal", label: () => t("sessions.level.minimal") },
  { value: "low", label: () => t("sessions.level.low") },
  { value: "medium", label: () => t("sessions.level.medium") },
  { value: "high", label: () => t("sessions.level.high") },
] as const;
const BINARY_THINK_LEVELS = [
  { value: "", label: () => t("sessions.level.inherit") },
  { value: "off", label: () => t("sessions.level.off") },
  { value: "on", label: () => t("sessions.level.on") },
] as const;
const VERBOSE_LEVELS = [
  { value: "", label: () => t("sessions.level.inherit") },
  { value: "off", label: () => t("sessions.level.off_explicit") },
  { value: "on", label: () => t("sessions.level.on") },
] as const;
const REASONING_LEVELS = [
  ...
  { value: "off", label: () => t("sessions.level.off") },
  { value: "on", label: () => t("sessions.level.on") },
  { value: "stream", label: () => t("sessions.level.stream") },
] as const;
  ...
  return html`
    <section class="card">
      ...
      <div class="filters" style="margin-top: 14px;">
        <label class="field">
          <span>${t("sessions.filter.active_minutes")}</span>
          <input
        ...
        <label class="field checkbox">
          <span>${t("sessions.filter.include_global")}</span>
          <input
            type="checkbox"
        ...
        <label class="field checkbox">
          <span>${t("sessions.filter.include_unknown")}</span>
          <input
            type="checkbox"
      ...
      <div class="muted" style="margin-top: 12px;">
        ${props.result ? t("sessions.store").replace("{path}", props.result.path) : ""}
      </div>
    ...
    <div class="table-row">
      ...
      <div>
        <input
          .value=${row.label ?? ""}
          ?disabled=${disabled}
          placeholder="${t("sessions.label.optional")}"
          @change=${(e: Event) => {
      ...
      <div>
        ...
        >
          ${thinkLevels.map((level) => html`<option value=${level.value}>${level.label()}</option>`)}
        </select>
      </div>
      <div>
        <select
