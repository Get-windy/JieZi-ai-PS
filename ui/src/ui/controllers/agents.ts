...
export async function loadAgents(state: AgentsState) {
  ...
  if (typeof window !== "undefined" && (window as unknown as { __DEBUG_UI__?: boolean }).__DEBUG_UI__) {
    console.log("[DEBUG:Agents:loadAgents] called:", {
      hasClient: !!state.client,
      connected: state.connected,
      alreadyLoading: state.agentsLoading,
    });
  }

  if (!state.client || !state.connected) {
    console.warn("[DEBUG:Agents:loadAgents] skipped: no client or not connected");
    return;
  }
  if (state.agentsLoading) {
    console.warn("[DEBUG:Agents:loadAgents] skipped: already loading");
    return;
  }
  state.agentsLoading = true;
  state.agentsError = null;
  try {
    console.log("[DEBUG:Agents:loadAgents] requesting agents.list...");
    const res = await state.client.request<AgentsListResult>("agents.list", {});
    console.log("[DEBUG:Agents:loadAgents] received response:", { agentsCount: res?.agents?.length ?? 0, defaultId: res?.defaultId });
    if (res) {
    ...
    console.error("[DEBUG:Agents:loadAgents] error:", err);
    ...
    console.log("[DEBUG:Agents:loadAgents] loading finished");
  }
}
