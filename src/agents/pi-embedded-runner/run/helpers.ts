...
export const DEFAULT_RATE_LIMIT_FAILOVER_BACKOFF_MS = 0;
...
export function resolveRateLimitFailoverBackoffMs(cfg?: OpenClawConfig): number {
  return cfg?.auth?.cooldowns?.rateLimitedBackoffMs ?? DEFAULT_RATE_LIMIT_FAILOVER_BACKOFF_MS;
}
