...
import {
  ...
  resolveRateLimitFailoverBackoffMs,
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      const rateLimitFailoverBackoffMs = resolveRateLimitFailoverBackoffMs(params.config);
      ...
      const maybeBackoffBeforeRateLimitFailover = async () => {
        if (rateLimitFailoverBackoffMs <= 0) {
          return;
        }
        log.warn(
          `rate-limit backoff before failover for ${provider}/${modelId}: delayMs=${rateLimitFailoverBackoffMs}`,
        );
        try {
          await sleepWithAbort(rateLimitFailoverBackoffMs, params.abortSignal);
        } catch (err) {
          if (params.abortSignal?.aborted) {
            const abortErr = new Error("Operation aborted", { cause: err });
            abortErr.name = "AbortError";
...
")}`
          ...
          if (promptError && !aborted && promptErrorSource !== "compaction") {
            ...
            if (promptFailoverReason === "rate_limit") {
              ...
              await maybeBackoffBeforeRateLimitFailover();
            }
          ...
          const assistantFailoverOutcome = await handleAssistantFailover({
            ...
            maybeBackoffBeforeRateLimitFailover,
            advanceAuthProfile,
          });
