...
export function createApprovalRequestTool(opts?: {
  ...
  return {
    ...
    description: "Create an approval request for an operation that requires approval from a supervisor or admin.",
    parameters: Type.Object({
      ...
      approvalLevel: Type.Optional(Type.Number({ 
 ...
 * 批准审批请求工具
 */
export function createApproveRequestTool(opts?: {
  currentAgentId?: string;
}): AnyAgentTool {
  return {
 ...
 * 拒绝审批请求工具
 */
export function createRejectRequestTool(opts?: {
  currentAgentId?: string;
}): AnyAgentTool {
  return {
 ...
 * 列出待审批请求工具
 */
export function createListPendingApprovalsTool(opts?: {
  currentAgentId?: string;
}): AnyAgentTool {
  return {
 ...
 * 获取审批状态工具
 */
export function createGetApprovalStatusTool(opts?: {
  currentAgentId?: string;
}): AnyAgentTool {
  return {
 ...
 * 取消审批请求工具
 */
export function createCancelApprovalRequestTool(opts?: {
  currentAgentId?: string;
}): AnyAgentTool {
  return {
    label: "Cancel Approval Request",
    name: "cancel_approval_request",
    description: "Cancel a pending approval request. Only the requester can cancel their own requests.",
    parameters: Type.Object({
      approvalId: Type.String({ 
