...
import { openBoundaryFile } from "../../upstream/src/infra/boundary-file-read.js";
...
async function readWorkspaceOnboardingState(statePath: string): Promise<WorkspaceSetupState> {
  try {
    const raw = await fs.readFile(statePath, "utf-8");
    const parsed = parseWorkspaceOnboardingState(raw);
    if (
      parsed &&
...
async function readWorkspaceOnboardingStateForDir(dir: string): Promise<WorkspaceSetupState> {
...
export async function isWorkspaceSetupCompleted(dir: string): Promise<boolean> {
  ...
  const completedAt =
    ...
    ((state as unknown as Record<string, unknown>).onboardingCompletedAt as string | undefined);
...
export async function ensureAgentWorkspace(params?: {
  ...
  if (
    !state.setupCompletedAt &&
    (state as unknown as Record<string, unknown>).onboardingCompletedAt === undefined &&
  ...
  if (
    !state.bootstrapSeededAt &&
    !state.setupCompletedAt &&
    !(state as unknown as Record<string, unknown>).onboardingCompletedAt &&
    !bootstrapExists
  ) {
