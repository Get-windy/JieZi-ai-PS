...
export class BootstrapLoader {
  ...
  private buildSystemPathDeclaration(agentId: string, workspaceDir: string): string {
    ...
    return [
      ...
      `- **项目空间根目录**：\`${groupsRoot}\``,
      `  - 每个项目的「项目空间」：\`${groupsRoot}\\{projectId}\`（存放 AI 记忆、文档、决策等协作文件）`,
      `  - 项目共享记忆：\`${groupsRoot}\\{projectId}\\SHARED_MEMORY.md\``,
      `  - 项目配置文件：\`${groupsRoot}\\{projectId}\\PROJECT_CONFIG.json\``,
      `  ⚠️  **项目空间 ≠ 业务空间**：项目空间用于存放 AI 协作文件（记忆/文档/决策），`,
      `     「业务空间」才是 AI 实际开展工作的目录（如代码仓库 I:\\AI-Ready、销售文件夹等），`,
      `     业务空间路径存储在 PROJECT_CONFIG.json 的 codeDir 字段，必须通过 project_list 工具获取。`,
      "",
      "**操作要求**：",
      "- 写文件时必须使用上述绝对路径，禁止猜测或使用相对路径",
      "- 业务文件必须写入项目的「业务空间」路径（codeDir，从 project_list 返回结果中读取），不得写入项目空间目录",
      "- 读取项目记忆前必须先用 `project_list` 工具获取真实 projectId，再拼接路径",
      "- 本区块由系统在每次会话启动时自动注入，内容以本次为准",
      "",
      "**启动自检三步（每次会话必须执行）**：",
      "1. 确认路径：核对以上「我的个人工作空间」路径与本机实际一致，若不一致立即告知用户",
      "2. 查询项目：调用 `project_list` 获取当前真实项目列表，重点记录每个项目的「业务空间」（codeDir）和「项目空间」（workspacePath），两者是完全不同的目录",
      "3. 校验层级：写入 MEMORY.md 前判断内容是否属于个人认知（✅）或项目成果（❌ → 应写 SHARED_MEMORY.md）；开展具体业务时确认操作的是业务空间而非项目空间",
      "",
      `*注入时间：${now}*`,
      "---",
    ].join("\n");
    ...
    const staticToolGroups: Array<{ group: string; tools: string[] }> = [
      {
        ...
        tools: [
          ...
          "task_worklog_add",
          "task_ping_activity",
          "task_progress_note_append",
          "task_report_to_supervisor",
          "task_reset",
        ],
      },
