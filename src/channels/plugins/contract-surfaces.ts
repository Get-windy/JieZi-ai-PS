...
// to avoid TDZ errors when bundled plugins are loaded via jiti and a circular
// import re-enters this module before its top-level bindings have been fully
// initialized. var has no TDZ: accessing it before initialization returns
...
// eslint-disable-next-line no-var
var __overlayRFBA: boolean | undefined;
function isRunningFromBuiltArtifact(): boolean {
  if (typeof __overlayRFBA === "undefined") {
    const p = getCurrentModulePath();
    __overlayRFBA =
      p.includes(`${path.sep}dist${path.sep}`) || p.includes(`${path.sep}dist-runtime${path.sep}`);
  }
  return __overlayRFBA;
}
...
function resolveSourceFirstContractSurfaceModulePaths(params: {
  ...
  const sourceRoot = path.resolve(getOpenClawPackageRoot(), "extensions", dirName);
  const modulePaths: string[] = [];
