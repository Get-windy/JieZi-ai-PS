...
import { checkMeetingModifyAccess } from "../../tasks/permissions.js";
...
export const meetingsRpc: GatewayRequestHandlers = {
  ...
  "meeting.create": async ({ params, respond }) => {
    try {
      const title = params?.title ? String((params as { title?: string | number }).title) : "";
      const description = params?.description
        ? String((params as StringifiableParams).description)
        : undefined;
      const organizerId = params?.organizerId
        ? String((params as { organizerId?: string | number }).organizerId)
        : "system";
      const organizerType = params?.organizerType
        ? (String((params as { organizerType?: string | number }).organizerType) as MemberType)
        : "human";
      const participantIds = params?.participantIds ? (params.participantIds as string[]) : [];
      const type = params?.type
        ? (String((params as { type?: string | number }).type) as MeetingType)
        : "other";
      const scheduledAt = params?.scheduledAt ? Number(params.scheduledAt) : 0;
      const duration = params?.duration ? Number(params.duration) : 60;
      const agenda = params?.agenda ? (params.agenda as unknown[]) : [];
      const organizationId = params?.organizationId
        ? String((params as StringifiableParams).organizationId)
        : undefined;
      const teamId = params?.teamId ? String((params as StringifiableParams).teamId) : undefined;
      const projectId = params?.projectId
        ? String((params as StringifiableParams).projectId)
        : undefined;
      ...
      if (!scheduledAt || scheduledAt < Date.now()) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "会议时间必须在未来"));
        return;
      }

      if (participantIds.length < 1) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "至少需要一名参会者"));
        return;
      }

      // 验证会议类型
      if (!["standup", "review", "planning", "brainstorm", "decision", "other"].includes(type)) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "无效的会议类型"));
        return;
      }
      ...
      // 创建议程列表
      const agendaItems: MeetingAgendaItem[] = agenda.map((item: unknown, index) => {
        const agendaItem = item as StringifiableParams;
        return {
          id: `agenda-${index}-${Date.now()}`,
          topic: String(agendaItem.topic || ""),
          description: agendaItem.description ? String(agendaItem.description) : undefined,
          duration: agendaItem.duration ? Number(agendaItem.duration) : 10,
          presenter: agendaItem.presenter ? String(agendaItem.presenter) : undefined,
      ...
      for (const participant of participants) {
        if (participant.id !== organizerId) {
          console.log(
            `[Meeting Notification] Invitation sent to ${participant.id} for meeting ${meetingId}`,
          );
          // 实际环境中应该调用通知系统发送会议邀请
        }
      }
  ...
  "meeting.update": async ({ params, respond }) => {
    try {
      const meetingId = params?.meetingId ? String((params as StringifiableParams).meetingId) : "";
      const title = params?.title ? String((params as StringifiableParams).title) : undefined;
      const description = params?.description
        ? String((params as StringifiableParams).description)
        : undefined;
      const scheduledAt = params?.scheduledAt ? Number(params.scheduledAt) : undefined;
      const duration = params?.duration ? Number(params.duration) : undefined;
      const agenda = params?.agenda ? (params.agenda as unknown[]) : undefined;

      if (!meetingId) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "会议ID不能为空"));
        return;
      }
      ...
      if (!meeting) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "会议不存在"));
        return;
      }

      // 权限检查 - 需要组织者权限
      const requesterId = params?.requesterId
        ? String((params as StringifiableParams).requesterId)
        : undefined;
      if (requesterId) {
        ...
        if (!permCheck.allowed) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, permCheck.reason || "无权限修改此会议"),
          );
          return;
        }
      }
      ...
      if (meeting.status !== "scheduled") {
        respond(
          ...
          errorShape(
            ...
            `只能更新scheduled状态的会议，当前状态: ${meeting.status}`,
          ),
        );
        return;
      }
      ...
      const updatedMeeting = await storage.updateMeeting(meetingId, {
        ...
        duration,
        agenda: agenda
          ? agenda.map((item: unknown, index: number) => ({
              id: `agenda-${index}-${Date.now()}`,
              topic: String((item as StringifiableParams).topic || ""),
              description: (item as StringifiableParams).description
                ? String((item as StringifiableParams).description)
                : undefined,
              duration: (item as StringifiableParams).duration
                ? Number((item as StringifiableParams).duration)
                : 10,
              presenter: (item as StringifiableParams).presenter
                ? String((item as StringifiableParams).presenter)
                : undefined,
              status: "pending" as const,
            }))
          : undefined,
      ...
      for (const participant of meeting.participants) {
        console.log(
          `[Meeting Notification] Meeting ${meetingId} updated, notifying ${participant.id}`,
        );
        // 实际环境中应该通知所有参会者
      }
  ...
  "meeting.cancel": async ({ params, respond }) => {
    try {
      ...
      const cancelReason = params?.cancelReason
        ? String((params as StringifiableParams).cancelReason)
        : "";
      ...
      if (requesterId) {
        ...
        if (!permCheck.allowed) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, permCheck.reason || "无权限取消此会议"),
          );
          return;
        }
      }
      ...
      for (const participant of meeting.participants) {
        console.log(
          `[Meeting Notification] Meeting ${meetingId} cancelled, notifying ${participant.id}`,
        );
        // 实际环境中应该通知所有参会者
      }
  ...
  "meeting.invite.respond": async ({ params, respond }) => {
    try {
      ...
      const participantId = params?.participantId
        ? String((params as StringifiableParams).participantId)
        : "";
      const response = params?.response
        ? (String((params as StringifiableParams).response) as MeetingResponse)
      ...
      if (!["accepted", "declined", "tentative"].includes(response)) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "无效的响应类型"));
        return;
      }
      ...
      if (!participant) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "不在参会者列表中"));
        return;
      }
      ...
      // 通知组织者
      console.log(
        `[Meeting Notification] ${participantId} responded ${response} to meeting ${meetingId}`,
      );
      // 实际环境中应该通知会议组织者
    ...
    try {
      ...
      if (requesterId) {
        ...
        if (!permCheck.allowed) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, permCheck.reason || "无权限开始此会议"),
          );
          return;
        }
      }
      ...
      if (meeting.status !== "scheduled") {
        respond(
          ...
          errorShape(
            ErrorCodes.INVALID_REQUEST,
            `只能开始scheduled状态的会议，当前状态: ${meeting.status}`,
          ),
        );
        return;
      }
      ...
      for (const participant of meeting.participants) {
        console.log(
          `[Meeting Notification] Meeting ${meetingId} started, notifying ${participant.id}`,
        );
        // 实际环境中应该通知所有参会者
      }
    ...
    try {
      ...
      if (requesterId) {
        ...
        if (!permCheck.allowed) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, permCheck.reason || "无权限结束此会议"),
          );
          return;
        }
      }
      ...
      if (meeting.status !== "in-progress") {
        respond(
          ...
          errorShape(
            ErrorCodes.INVALID_REQUEST,
            `只能结束in-progress状态的会议，当前状态: ${meeting.status}`,
          ),
        );
        return;
      }
  ...
  "meeting.message.send": async ({ params, respond }) => {
    try {
      const meetingId = params?.meetingId ? String((params as StringifiableParams).meetingId) : "";
      const senderId = params?.senderId ? String((params as StringifiableParams).senderId) : "";
      const senderType = params?.senderType
        ? (String((params as StringifiableParams).senderType) as MemberType)
        : "human";
      const content = params?.content ? String((params as StringifiableParams).content) : "";
      const messageType = params?.messageType
        ? String((params as StringifiableParams).messageType)
        : "text";
      const replyToMessageId = params?.replyToMessageId
        ? String((params as StringifiableParams).replyToMessageId)
        : undefined;
      ...
      // 验证消息类型
      if (!["text", "decision", "action-item", "poll", "file"].includes(messageType)) {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "无效的消息类型"));
        return;
      }
      ...
      if (!isParticipant && senderId !== meeting.organizerId) {
        respond(false, undefined, errorShape(ErrorCodes.UNAVAILABLE, "不在参会者列表中"));
        return;
      }

      // 验证会议正在进行中
      if (meeting.status !== "in-progress") {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "会议未在进行中"));
        return;
      }
      ...
      const newMessage: MeetingMessage = {
        ...
        senderType,
        content,
        messageType: messageType as "text" | "decision" | "action-item" | "poll" | "file",
        replyToMessageId,
        createdAt: Date.now(),
      };
      ...
      if (requesterId) {
        ...
        if (!permCheck.allowed) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.UNAVAILABLE, permCheck.reason || "无权限切换议题"),
          );
          return;
        }
      }
  ...
  "meeting.decision.record": async ({ params, respond }) => {
    try {
      const meetingId = params?.meetingId ? String((params as StringifiableParams).meetingId) : "";
      const content = params?.content ? String((params as StringifiableParams).content) : "";
      const proposedBy = params?.proposedBy
        ? String((params as StringifiableParams).proposedBy)
        : "";
      const approvedBy = params?.approvedBy ? (params.approvedBy as string[]) : undefined;
      const agendaItemId = params?.agendaItemId
        ? String((params as StringifiableParams).agendaItemId)
        : undefined;
      const impact = params?.impact
        ? (String((params as StringifiableParams).impact) as "high" | "medium" | "low")
        : undefined;
      ...
      for (const participant of meeting.participants) {
        console.log(
          `[Meeting Notification] Decision recorded in meeting ${meetingId}, notifying ${participant.id}`,
        );
        // 实际环境中应该通知所有参会者
      }
  ...
  "meeting.actionitem.create": async ({ params, respond }) => {
    try {
      ...
      const assigneeId = params?.assigneeId
        ? String((params as StringifiableParams).assigneeId)
        : "";
      const assigneeType = params?.assigneeType
        ? (String((params as StringifiableParams).assigneeType) as MemberType)
        : "agent";
      const dueDate = params?.dueDate ? Number(params.dueDate) : undefined;
      const priority = params?.priority
        ? (String((params as StringifiableParams).priority) as TaskPriority)
        : "medium";
      const agendaItemId = params?.agendaItemId
        ? String((params as StringifiableParams).agendaItemId)
        : undefined;
      ...
      // 通知被分配者
      console.log(
        `[Meeting Notification] Action item assigned to ${assigneeId} in meeting ${meetingId}`,
      );
      // 实际环境中应该通知被分配者
  ...
  "meeting.notes.generate": async ({ params, respond }) => {
    try {
      ...
      const generatedBy = params?.generatedBy
        ? String((params as StringifiableParams).generatedBy)
        : "system";
      ...
      if (meeting.status !== "completed") {
        respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "会议尚未结束"));
        return;
      }
      ...
      // 保存会议纪要（添加到会议对象中）
      // 注意：summary字段可能不在Meeting类型定义中，使用类型断言
      await storage.updateMeeting(meetingId, { summary } as unknown as Partial<Meeting>);
...
async function generateAIMeetingSummary(
  ...
  meeting.decisions.forEach((decision, index) => {
    if (index < 3) {
      // 最多提取3个决策
      keyPoints.push(
        `决策${index + 1}: ${decision.content.substring(0, 50)}${decision.content.length > 50 ? "..." : ""}`,
      );
    }
  });
