...
export const agentsManagementHandlers: GatewayRequestHandlers = {
  ...
  /**
   * agent.task.manage - 主控对任务进行干预：取消、重置、延时或拆分
   *
   * 适用场景：
   * - cancel: 任务长时间阻塞无输出，判断无法完成，直接取消
   * - reset:  任务卡住但値得重试，重置回 todo 重新排队
   * - extend: 任务因难度较高超时，延长超时阈値（记录在 metadata）
   * - split:  将阻塞大任务拆分为多个子任务（每个小范围可独立推进），并取消原任务
   *
  ...
  "agent.task.manage": async ({ params, respond }) => {
    const p = params as {
      taskId: string;
      action: "cancel" | "reset" | "extend" | "split";
      ...
      subtasks?: Array<{
        title: string;
        description?: string;
        assigneeId?: string; // 不填则继承原任务 assignee
        priority?: string;
      }>;
    };
    ...
    if (!taskId || !["cancel", "reset", "extend", "split"].includes(action)) {
      respond(
        ...
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          "taskId and action (cancel|reset|extend|split) are required",
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (action === "cancel") {
        ...
        // === 拆分任务：将大任务拆为多个子任务 ===
        const subtaskDefs = Array.isArray(p?.subtasks) ? p.subtasks : [];
        if (subtaskDefs.length < 2) {
          respond(
            false,
            undefined,
            errorShape(ErrorCodes.INVALID_REQUEST, "split action requires at least 2 subtasks"),
          );
          return;
        }

        const defaultAssigneeId = task.assignees?.[0]?.id ?? "";
        ...
        for (const def of subtaskDefs) {
          ...
          const subtask = {
            ...
            priority: (def.priority ?? task.priority) as Task["priority"],
            organizationId: task.organizationId,
            teamId: task.teamId,
            projectId: task.projectId,
            supervisorId: task.supervisorId,
            timeTracking: { timeSpent: 0, lastActivityAt: now },
            createdAt: now,
          };
          await taskStorage.createTask(subtask);
          createdSubtaskIds.push(subId);

          // 通知子任务 assignee
          if (subAssigneeId) {
            const normalizedSub = normalizeAgentId(subAssigneeId);
            enqueueSystemEvent(
              [
                `[TASK ASSIGNED] A new subtask has been assigned to you (split from ${taskId}).`,
                `Subtask ID: ${subId}`,
                `Title: ${subtask.title}`,
            ...
            requestHeartbeatNow({
              reason: `manage:split:${subId}`,
              sessionKey: `agent:${normalizedSub}:main`,
              agentId: normalizedSub,
              coalesceMs: 5000,
            });
          }
        }
        ...
        await taskStorage.updateTask(taskId, {
          subtasks: [...existingSubtasks, ...createdSubtaskIds],
          status: "cancelled",
          cancelledAt: now,
          cancelReason: `Split into ${createdSubtaskIds.length} subtasks by ${operatorId}. ${reason}`,
        });
        await taskStorage.addWorklog({
          id: `wl_${now}_${Math.random().toString(36).slice(2, 9)}`,
          taskId,
          agentId: operatorId,
          action: "split",
          details: `[Split] Task split into ${createdSubtaskIds.length} subtasks: ${createdSubtaskIds.join(", ")}. Reason: ${reason || "task too large or blocked"}`,
          result: "partial",
          createdAt: now,
        });
        ...
        respond(
          true,
          { success: true, taskId, action: "split", subtaskIds: createdSubtaskIds },
          undefined,
        );
      }
    } catch (err) {
