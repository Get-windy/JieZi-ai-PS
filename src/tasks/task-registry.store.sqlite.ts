...
function bindTaskRecordBase(record: TaskRecord) {
  ...
  // If a required NOT NULL column is undefined/object, the driver throws
  // "Provided value cannot be bound to SQLite parameter N". Coerce to safe
  // defaults here so a single bad record never aborts the whole transaction.
  const runtime: string = typeof record.runtime === "string" && record.runtime
    ? record.runtime
    : "subagent";
  const ownerKey: string = typeof record.ownerKey === "string" && record.ownerKey
    ? record.ownerKey
    : record.taskId;
  const scopeKind: string = typeof record.scopeKind === "string" && record.scopeKind
    ? record.scopeKind
    : "session";
  const task: string = typeof record.task === "string"
    ? record.task
    : typeof record.task === "object" && record.task !== null
      ? JSON.stringify(record.task)
      : String(record.task ?? "");
  const status: string = typeof record.status === "string" && record.status
    ? record.status
    : "queued";
  const deliveryStatus: string = typeof record.deliveryStatus === "string" && record.deliveryStatus
    ? record.deliveryStatus
    : "pending";
  const notifyPolicy: string = typeof record.notifyPolicy === "string" && record.notifyPolicy
    ? record.notifyPolicy
    : "done_only";
  return {
    ...
    requester_session_key: scopeKind === "system" ? "" : (record.requesterSessionKey ?? ownerKey),
    owner_key: ownerKey,
